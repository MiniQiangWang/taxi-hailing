<template>
  <div class="bank">
    ...
    <div class="cardInfo_content">
        ...
        <div><span class="icon_time"></span>下单时间 {{curPMsg.o_date}}</div>
        <div><span class="icon_blue"></span>起点 {{curPMsg.start_place}}</div>
        <div><span class="icon_org"></span>终点 {{curPMsg.end_place}}</div>
    </div>
    <!--内容结束-->
...
export default {
    ...
    computed: {
        curPMsg() {
            return this.$store.state.orderList.curPMsg;
        },
    },
    mounted() {
        ...
        console.log('lajeogb',this.curPMsg.start_place)
        driving.search(this.curPMsg.start_place,this.curPMsg.end_place);
    },
    methods:{
